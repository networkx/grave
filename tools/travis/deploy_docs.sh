...
then
    ...
    #
    # $ ssh-keygen -t ed25519 -C "Grave Travis Bot" -f deploy-key
    ...
    openssl aes-256-cbc -K $encrypted_07cb038099db_key -iv $encrypted_07cb038099db_iv -in tools/travis/deploy-key.enc -out deploy-key -d
    chmod 600 deploy-key
    eval `ssh-agent -s`
    ssh-add deploy-key

    # Push the latest docs to the networkx/grave repo (gh-pages branch)
    GH_REF=git@github.com:networkx/grave.git
    echo "-- pushing docs --"
    (
    git config --global user.email "travis@travis-ci.com"
    git config --global user.name "Grave Travis Bot"
    ...
    git rm -r latest
    cp -a ../_build/html latest
    touch latest/_downloads
    cp -a ../_build/latex/grave.pdf latest/_downloads/.
    git add latest
